...
int main(int argc, char* argv[]){
  ...
  for(i=0;i<N_CALC;i++){
    ...
    M2_min = (double)i * 1.5 / (double)N_CALC;
    ...
    if(ADD_NS) P1 += prob_NS(NS_RATE,M1,M2_min);
    

    // Need to integrate over all M2_min
    qromb(prob_M2_wrapper,0.001,NS_M,1.0e-4,&P2,model);
    if(ADD_NS) P2 += prob_NS_all(NS_RATE,M1);
    OUT << M2_min << " " << P1/P2*5 << endl;
